...
public class VediCarrelloServlet extends HttpServlet{
	@Override
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			List<Carrello_Prodotto> cp =  sv.findCarrelloProdottiUtente((Utente)req.getSession().getAttribute(Costanti.UTENTE));
			aggiungiImmaggini(cp, req);
			
			req.setAttribute(Costanti.CARRELLO, cp);
	...
	public void aggiungiImmaggini(List<Carrello_Prodotto> prodotti, HttpServletRequest request) throws Exception {
		...
		for (Carrello_Prodotto prodotto : prodotti) {
			Blob immagine = prodotto.getProdotto().getFoto();
			if (immagine != null) {
				String filePath = uploadPath + File.separator + prodotto.getProdotto().getId() + "_" + prodotto.getProdotto().getNomeImmagine();
				BlobConverter.saveFile(immagine, filePath);
				String imageUrl = baseHttpUrl + File.separator + Costanti.UPLOAD_DIRECTORY + File.separator + prodotto.getProdotto().getId() + "_" + prodotto.getProdotto().getNomeImmagine(); 
				mappa.put(prodotto.getProdotto().getId(), imageUrl);
			} else {
