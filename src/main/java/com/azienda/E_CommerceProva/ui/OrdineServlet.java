...
public class OrdineServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			List<Carrello_Prodotto> carrello_Prodottos = sv.findCarrelloProdottiUtente(u);
			
			sv.acquisto(carrello_Prodottos, u);
			req.getSession().setAttribute(Costanti.CARRELLO, sv.findCarrelloProdottiUtente(u));
			
			sv.sendEmail(u, carrello_Prodottos);
			
			Thread.sleep(4000);
			
			resp.sendRedirect("http://" + req.getServerName() + ":" + req.getServerPort() + req.getContextPath() + "/Jsp/RingraziamentoAcquisto.jsp");	
