...
@WebServlet("/EliminaFornitore")
public class EliminaFornitoreServlet extends HttpServlet{
	@Override
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			Fornitore c = sv.findFornitoreById(Integer.parseInt((req.getParameter(Costanti.FORNITORE_ID))));
		
			sv.deleteFornitore(c);
			
			req.getRequestDispatcher("/Fornitore").forward(req, resp);
								
			
		}catch (FornitoreNonVuotoException e) {
			e.printStackTrace();
