...
<jsp:include page="navbar2.jsp"></jsp:include>
   ...
   <div class="container">
        ...
        <div class="welcome-message">Sei nella Modifica Categoria</div>
      
         <p id="info"> Qui sotto puoi modificare le tue categorie </p>
...
<%
		...
		Categoria p = (Categoria) request.getAttribute(Costanti.CATEGORIA);
		%>
		<div><%=messageError%></div>
		
    <div class="container" id="contabella" style="padding-bottom: 17rem;">
       <form action="<%=request.getContextPath() +"/ModificaCategoria" %>" method="post" enctype="multipart/form-data">
      	 	<div class="form-group">
                <label for="id">ID:</label>
                <input type="text" id="id" name="<%=Costanti.CATEGORIA_ID%>" value="<%=p.getId()%>" readonly="readonly">
            </div>
            <div class="form-group">
                <label for="nome">Nome:</label>
                <input type="text" id="nome" name="<%=Costanti.CATEGORIA_NOME %>" value="<%=p.getNome()%>">
            </div>
               <div class="form-group">
                <label for="foto">FOTO:</label>
                <input type="file" id="foto" name="<%=Costanti.CATEGORIA_LOGO %>">
            </div>
            <div style="width: 100%">
                <input id="modificacategoria" class="subbtn" type="submit" value="Modifica categoria">
                
             <a class="btn btn-primary" style="margin-top: 2vh; margin-left: 56vw; width: 18%" href="<%= "http://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath() + "/Categoria"%>">Torna alle categorie</a>
            </div>
        </form>
    </div>
