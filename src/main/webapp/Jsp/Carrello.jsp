...
body {
visibility: hidden; /* Nascondi il contenuto */
...
<jsp:include page="navbar2.jsp"></jsp:include>
	<% 
        ...
        List<Carrello_Prodotto> carrello = (List<Carrello_Prodotto>) request.getAttribute(Costanti.CARRELLO);
	...
	<div class="container cart">
		<table>
			...
			<%
			for (Carrello_Prodotto cp : carrello) {
        %>
			<tr>
				<td>
					<div class="cart-info">
						<a href="VediProdotto?<%=Costanti.PRODOTTO_ID%>=<%=cp.getProdotto().getId()%>">
							<img alt="" src="<%=mappa.get(cp.getProdotto().getId()) %>" width="100" height="100">
						</a>
						<div>
							<p><%=cp.getProdotto().getNome() %></p>
							<span>Prezzo: <%=cp.getProdotto().getPrezzo()%>&euro;</span>
							<form action="<%=request.getContextPath() + "/EliminaProdottoCarrello"%>">					
						    <input type="hidden" name=<%=Costanti.PRODOTTO%> value=<%=cp.getId()%>>
						    <button type="submit" class="remove btn" data-id="<%=cp.getProdotto().getId() %>">Rimuovi Prodotto</button>					 
						 </form>
					
							<%
            					double prezzoProdotto = cp.getProdotto().getPrezzo() * cp.getQuantita();
				...
				<td><%=String.format("%.2f", prezzoProdotto)%>&euro;</td>
			</tr>
			<% } %>
		</table>
		<div class="total-price">
			<div style="width: 100%">
            <a class="btn btn-primary" href="<%= "http://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath() + "/Catalogo"%>">Prosegui con gli acquisti</a>
            </div>
			<table>
				<tr>
					<td>Totale&nbsp;&nbsp;&nbsp;&nbsp;<%=String.format("%.2f", prezzoTotale)%>&euro;</td>
				</tr>
			</table>
			
				<form action="<%=request.getContextPath() + "/Ordine"%>">
					<button class="order" id="ok">
                            <span class="default">Completa Ordine</span>
                            <span class="success">Ordine Effettuato<svg viewBox="0 0 12 10">
...
<%
    ...
    %>
    <div class="kaka" style="padding-bottom: 10rem; ">
    <img alt="" src="<%=request.getContextPath() %>/Media/angrycat.png" style="height: 80%;">
    <div style="color: white; padding-bottom: 1rem;">Nessun prodotto presente nel carrello</div>
    <a class="btn btn-primary" href="<%= "http://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath() + "/Catalogo"%>">Torna nel catalogo</a>
    
    </div>
    <jsp:include page="footer.jsp"></jsp:include>
    <script>	
    document.querySelector('.navbar-brand').style.paddingLeft = '3.6rem';
    document.getElementById('40ladroni').style.paddingLeft = 0;  
    </script>
    <% } %>
    <script>// Quando la pagina viene caricata o ricaricata
 // Ritarda l'inizio del caricamento per 1 secondo
    setTimeout(function() {
        // Mostra il contenuto quando è pronto
        ...
        // Memorizza la posizione dello scroll quando l'utente scorre
...
<script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.4.1/jquery.min.js'></script><script> $('.order').click(function(e) {
		document.getElementById("ok").style.background = 'var(--dark)';
	    let button = $(this);
