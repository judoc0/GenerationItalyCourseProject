...
<jsp:include page="navbar2.jsp"></jsp:include>
   ...
   <div class="container">
        ...
        <div class="welcome-message">Sei nella Lista Fornitori </div>
      
         <p id="info"> Qui sotto puoi vedere la lista dei fornitori </p>
         
          <div id="bottoni">
          
          <a
		href="<%="http://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath()
		+ "/Jsp/CreaFornitore.jsp"%>">
		<button type="submit" id="categoria" class="subbtn">Crea fornitore </button></a>
	...
	<%
	List<Fornitore> categorie = (List<Fornitore>) request.getAttribute(Costanti.LISTA_FORNITORI);
	...
	<table border="1">
		<tr>
			<th style="font-size: 23px"><div id="nomediv">Nome </th>
			<th style="font-size: 23px"><div id="nomediv">Logo</th>
		</tr>

		<%
		for (Fornitore cat : categorie) {
		%>

		<tr>
			<td style="font-size: 20px"> <div id="nomediv"> <%=cat.getNome()%> </div></td>
			<td> <div id="nomediv"> <img alt="" src="<%=mappa.get(cat.getId())%>"
					width="100" height="100"> </div></td>
			
		<td>
		<form action="<%=request.getContextPath()+"/ModificaFornitoreRedirect"%>">
			<input type="hidden" name=<%=Costanti.FORNITORE_ID%> value=<%=cat.getId()%>>
			<button type="submit"  id="fornitori2" class="subbtn">Modifica fornitore</button>
		</form>		
		</td>
		<td>
		<form action="<%=request.getContextPath()+"/EliminaFornitore"%>">
			<input type="hidden" name=<%=Costanti.FORNITORE_ID%> value=<%=cat.getId()%>>
			<button type="submit" id="fornitori" class="subbtn">Elimina fornitore</button>
		</form>		
		</td>	
	...
	%>
	<div>Nessun fornitore presente nel catalogo</div>
	<a href="http://localhost:8080/E_CommerceProva/Catalogo">Torna nel catalogo</a>
	<%
	}
	%>
