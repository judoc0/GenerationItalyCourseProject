...
.custom-page-brand {
    padding-left: 3.6rem; /* Nuovo valore di padding-left */
}
...
<jsp:include page="navbar2.jsp"></jsp:include>
	<% 
		...
		List<Ordine> ordines = (List<Ordine>) request.getAttribute(Costanti.LISTA_ORDINI);
      
        if (ordines.size() > 0) {
     
            
        %>
    <div class="container cart" id="pepega">
    	<table>
    		<tr>
				<th>ID ORDINE</th>
				<th>DATA</th>
			...
			for (Ordine o : ordines) {
				LocalDateTime inputDateTime = o.getDate();			
        %>
        	
			<tr>
				<td style="color: white;"><%=o.getId() %></td>
				<td style="color: white;"><%=inputDateTime.format(DateTimeFormatter.ofPattern("dd-MM-yyyy"))  %></td>
				<td style="color: white;"><%=inputDateTime.format(DateTimeFormatter.ofPattern("HH:mm:ss"))%></td>
				<td></td>				
				<td>
				<div>
				<form action="<%=request.getContextPath() + "/VediOrdiniProdotti"%>">
				<input type="hidden" name="<%=Costanti.LISTA_ORDINI %>" value="<%=o.getId()%>" />
				<button type="submit" class="remove btn" style="margin: 0">Vedi prodotti</button>
				</form>
				</div>			
				</td>				
			</tr>
		...
		<a class="btn btn-primary" style="margin-top: 2vw" href="<%= "http://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath() + "/Utente"%>">Torna nella tua pagina personale</a>		
    </div>
...
<%
    ...
    <div style="color: white; padding-bottom: 1rem;">Nessun ordine presente</div>
    <a class="btn btn-primary" href="<%= "http://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath() + "/Utente"%>">Torna nella tua pagina personale</a>
    </div>
    <jsp:include page="footer.jsp"></jsp:include>
    <% } %>
    <%
    ...
    %>  
	<script>
		document.getElementById("pepega").style.minHeight = "46.4vh"; 
		
	</script>
	<%} else { %>
		<script>
		document.querySelector('.navbar-brand').style.paddingLeft = '4.1rem';
		document.querySelector('.asdrubale').style.paddingTop = "0.2vh";
		document.querySelector('.sociallink').style.paddingTop = "0.2vh";
	    document.getElementById('40ladroni').style.paddingLeft = "1rem"; 
	    document.getElementById('40ladroni').style.height = "9.4vh";
		</script>
	<% }%>
